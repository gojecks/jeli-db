...
class DatabaseSyncConnector {
    constructor(config, isExistingDB) {
        var activeDB = DatabaseSyncConnector.coreApi.getActiveDB(config.name);
        var resolver = activeDB.get(DatabaseSyncConnector.coreApi.constants.RESOLVERS);
                ...
                syncHelper.pullResource(config.name)
                    .then(function (response) {
                        ...
                        if (checkPassed && !$deleteManager.isDeletedDataBase()) {
                             ...
                             */
                            if (activeDB.get(DatabaseSyncConnector.coreApi.constants.RESOURCEMANAGER).getDataBaseLastSyncDate()) {
                                syncHelper.setMessage("Database doesn't exists on the server");
                                syncHelper.killState(config.name);
                                return DatabaseSyncConnector.coreApi.removeDB(config.name, true);
                            }
